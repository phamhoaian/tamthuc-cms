<div class="btn-back">
	<?php echo $this->Html->link('<i class="fa fa-arrow-left"></i> Quay lại', array('controller' => 'admin_news', 'action' => 'admin_index'), array('class' => 'btn btn-circle btn-primary', 'escape' => false)) ?>
</div>
<div class="row">
	<div class="col-md-12">
		<div class="portlet box green">
			<div class="portlet-title">
				<div class="caption">
					<i class="icon-plus"></i>Thêm tin mới
				</div>
			</div>
					...
					<div id="tab_content" class="tab-pane active fade in">
						<div class="form-body">
							<div class="form-group">
								<label class="col-md-2 control-label"><?php echo __('Thể loại'); ?> <span class="required" aria-required="true">*</span></label>
								<div class="col-md-2">
									<?php echo $this->Form->select('cat_id', $news_categories, array('class' => 'form-control', 'label' => false, 'required' => false, 'empty' => 'Chọn thể loại')) ?>
									<?php echo $this->Form->error('cat_id'); ?>
								</div>
							</div>
							<div class="form-group">
							...
							<div class="form-group">
								...
								<div class="col-md-5">
									<div class="uploading horizontal <?php echo ! empty($this->request->data["News"]["top_pic"]) && ! empty($this->request->data["News"]["top_pic"]["file_size"]) ? 'opened' : '';  ?>">
										<div class="preview-area">
											<div id="top_pic_preview" class="preview-wrap <?php echo ! empty($this->request->data["News"]["top_pic"]) && ! empty($this->request->data["News"]["top_pic"]["file_size"])  ? 'opened' : '';  ?>">
												<?php if (! empty($this->request->data["News"]["top_pic"]) && ! empty($this->request->data["News"]["top_pic"]["file_size"])) { ?>
													<div style="background: url(<?php echo $this->ImageLabel->url($this->request->data["News"]["top_pic"]); ?>) no-repeat center; background-size: contain; height: 208px; width: auto; margin: auto; border-radius: 10px;"></div>
												<?php } ?>
											</div>
										</div>
										<div class="drap-and-drop <?php echo ! empty($this->request->data["News"]["top_pic"]) && ! empty($this->request->data["News"]["top_pic"]["file_size"]) ? 'opened' : '';  ?>" data-file-id="top_pic">
											<label for="top_pic">
												<input type="file" id="top_pic" name="data[News][top_pic]" class="upload-file" data-preview-id="top_pic_preview" accept=".jpg,.jpeg,.gif,.png"  <?php if(!empty($readOnly)) echo 'disabled="true"' ?> data-type="image"/>
