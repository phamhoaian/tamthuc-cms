<div class="bread">
    <?php echo $this->Html->link(__('ユーザ一覧'), '/admin/admin_users/') ?> &gt;
    <?php echo __('ユーザー編集'); ?>
</div>
...
<div class="container">
    ...
    <table class="table table-bordered">
        ...
        <tr>
            <th><?php echo __('リターン受け取り先住所'); ?></th>
            <td>
                <?php echo $this->Form->input('receive_address') ?>
            </td>
        </tr>
        <tr>
            <th><?php echo __('データ作成日時'); ?></th>
            <td><?php echo date('Y/m/d H:i', strtotime($u['created'])) ?></td>
        </tr>
        <tr>
            <th><?php echo __('データ更新日時'); ?></th>
            <td><?php echo date('Y/m/d H:i', strtotime($u['modified'])) ?></td>
        </tr>
    </table>
    <button type="submit" class="btn btn-primary"><?php echo __('更新'); ?></button>
